...
this project is adapted from a class assignment in COMP 251 at McGill
 builds a hashtable using chaining method */
public class Chaining {
    ...
    public int chain (int key) {
        ...
        int hash = ((this.A * key) % ((int) Math.pow(2, this.w)) >> (w-r));
        return hash;
    }
    ...
    public int insertKey(int key){
        ...
        int index1 = chain(key);
        int collisions = 0;
        if (Table.get(index1) != null){
            for (int a = 0; a < Table.get(index1).size(); a++){
                collisions += 1; //if the slot list already has values, add to collions
            }
        }
        Table.get(index1).add(key);
        return collisions;
    }
